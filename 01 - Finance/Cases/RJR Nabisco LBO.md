---
title: RJR Nabisco LBO
tags: [case-study, finance]
---

# 📚 RJR Nabisco LBO

> **Core Lesson**: Leveraged buyouts, private equity

---

## 📋 Overview

| Attribute | Detail |
|-----------|--------|
| **Subject** | Finance |
| **Core Lesson** | Leveraged buyouts, private equity |
| **Source** | HBS / Top MBA Case |

---

## 🕰️ Background

In 1988, RJR Nabisco was a $25B tobacco and food conglomerate. CEO Ross Johnson launched a management buyout (MBO) at $75/share. KKR entered a bidding war, ultimately winning at $109/share ($25B) — making it the largest LBO in history at that time. The deal was financed with ~$20B in debt (junk bonds, bank debt) and ~$5B equity.

---

## ❓ The Central Problem

Was the RJR LBO value-creating or value-destroying? The bidding war drove the price far beyond initial estimates. Key tensions: management self-dealing (Johnson's MBO), board duty to shareholders, the role of junk bond financing (Drexel Burnham / Mike Milken), and whether LBO-driven restructuring creates real operational value or just financial engineering.

---

## 📊 Analysis

KKR's plan: sell non-core assets (Nabisco food brands), use cash flow to pay down debt, improve operational efficiency. Johnson's plan was self-serving — management would retain equity at a low price. The board hired Dillon Read and Lazard for fairness opinions. KKR ultimately won partly because their bid was "cleaner" (higher cash, less management enrichment). Post-deal: KKR struggled with the debt load; sold assets aggressively. Eventually profitable but returns were below KKR's typical IRR targets.

---

## 🔑 Key Lessons

1. **LBO bidding wars can destroy buyer returns** — KKR overpaid relative to its typical discipline
2. **Management buyouts create fiduciary conflicts** — board must protect all shareholders, not just management
3. Junk bond financing enabled unprecedented deal sizes but created systemic risk (Drexel collapsed in 1990)
4. The deal popularized the LBO model globally and remains the reference case for PE strategy

---

## 🎓 Discussion Questions

1. Was the RJR board right to accept KKR's higher bid over management's? What factors beyond price should matter?
2. How does the RJR case illustrate the agency problem between management and shareholders?
3. Was the massive use of junk bonds socially beneficial or did it create excessive financial risk?

---

## �� Connected Concepts

[[LBO Model]], [[Capital Structure]], [[Corporate Governance]], [[Investment Banking Overview]], [[M&A Strategy]]

---

*← [[📊 Finance MOC]] | [[📚 Case Studies MOC]]*